...
class TBThemes_Demo_Content {
    ...
    static function log_to_console($data) {
        if (is_array($data)) {
            $data = implode(',', $data);
            echo "<script>console.log('" . esc_js($data) . "');</script>";
    ...
    static function get_update_keys() {
        ...
        $r = wp_remote_post( admin_url( 'customize.php' ), array(
                ...
                'headers' => array(),
                'cookies' => array(
                    SECURE_AUTH_COOKIE => isset($_COOKIE[ SECURE_AUTH_COOKIE ]) ? $_COOKIE[ SECURE_AUTH_COOKIE ] : '',
...
// Support Upload XML and JSON files
